...
# Create and display quiz questions and radio buttons with horizontal options
for i in range(len(questions)):
    question_label = tk.Label(root, text=f"{i + 1}. {questions[i]}", padx=10, pady=5, bg='#E1D9D1', fg='#4E595D')
    question_label.pack()

    options_frame = tk.Frame(root, bg='#E1D9D1')
    options_frame.pack()

    for j in range(len(options[i])):
        radio_button = tk.Radiobutton(options_frame, text=options[i][j], variable=var_list[i], value=j, bg='#E1D9D1', fg='#1E3D59', activebackground='#1E3D59', activeforeground='#E1D9D1')
